...
def test_logging():
    ...
    mock_batch = Batch(
        text_inputs={'input_ids': torch.ones(1, 128), 'attention_mask': torch.ones(1, 128)},  # Match BERT input size
        video_frames=torch.ones(1, 3, 30, 224, 224),  # Match video frame shape
        audio_features=torch.ones(1, 64, 300)  # Match audio feature shape
    )
